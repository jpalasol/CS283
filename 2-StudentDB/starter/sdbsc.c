...
{
    // TODO
    student_t studentSpace = {0};
    off_t offset = id  * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) < 0){
        return ERR_DB_FILE;
    }
    int bytes_read = read(fd, &studentSpace, STUDENT_RECORD_SIZE);
    if (bytes_read < 0){
        return ERR_DB_FILE;
    }
    if (bytes_read == 0){
    ...
    memcpy(s, &studentSpace, STUDENT_RECORD_SIZE);
    int studentSearch = memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
    if (studentSearch == 0) {
        return SRCH_NOT_FOUND;
    }
    //  else {
    //     memcpy(s, &studentSpace, sizeof(student_t));
    // }
    
    // else {
    //     s->id <- studentSpace.id;
    //     s->gpa <- studentSpace.gpa;
    //     strcpy(s->fname, studentSpace.fname);
    //     strcpy(s->lname, studentSpace.lname);
...
{
    // TODO
    student_t stud_buffer = {0};
    off_t student_offset = id * STUDENT_RECORD_SIZE;

    if ((get_student(fd, id, &stud_buffer))== NO_ERROR) {
    ...
    if (lseek(fd, student_offset, SEEK_SET) < 0){
        ...
        newStudent.gpa = gpa;
        if (write(fd, &newStudent, STUDENT_RECORD_SIZE) < 0) {
    ...
    if (read(fd, &stud_buffer, STUDENT_RECORD_SIZE) < 0){
        printf(ERR_DB_FILE);
...
int del_student(int fd, int id)
{
    ...
    student_t no_student = {0};
    int bytes_written = write(fd, &no_student, STUDENT_RECORD_SIZE);
...
int count_db_records(int fd)
{
    ...
    while (bytes_read != 0) {
        if (studentSpace.id != 0) {
            counter++;
        }
        bytes_read = read(fd, &studentSpace, STUDENT_RECORD_SIZE);
    }
...
int print_db(int fd)
{
    ...
    while (bytes_read != 0) {
        if (studentSpace.id != 0) {
            printf(STUDENT_PRINT_FMT_STRING, studentSpace.id, studentSpace.fname, studentSpace.lname, studentSpace.gpa/100.00);
        }
        bytes_read = read(fd, &studentSpace, STUDENT_RECORD_SIZE);
    }
    return NO_ERROR;
...
void print_student(student_t *s)
{
    ...
    printf(STUDENT_PRINT_FMT_STRING, s->id, s->fname, s->lname, s->gpa/100.00);
    return;
}
